    ...
    for (const hasHistory of [true, false]) {
      ...
      for (const preset of ['paper', 'midnight']) {
        ...
        for (const width of [320, 390, 768, 1024, 1920]) {
          ...
          await home.waitForFunction(() => document.querySelector('.guide-scroll-hint')?.dataset.scrolled === 'false')
          const cover = await home.locator('.guide-cover').boundingBox()
          const content = await home.locator('.guide-content').boundingBox()
          const brand = await logo.boundingBox()
          const hint = await home.locator('.guide-scroll-hint').boundingBox()
          assert.ok(content.y >= 899, 'guide content stays below the initial viewport in normal flow')
          assert.ok(cover.height >= 800 && cover.y + cover.height >= 899, 'cover fills the available first screen')
          assert.ok(Math.abs(brand.y + brand.height / 2 - 450) < 70, 'logo sits near the viewport center')
          assert.ok(Math.abs(brand.width / brand.height - 480 / 214) < 0.02, 'logo retains its aspect ratio')
          assert.ok(hint.y > brand.y + brand.height && hint.y + hint.height < 880, 'hint stays below the logo and above the safe bottom margin')
          assert.equal(await home.locator('.guide-masthead').evaluate(e => getComputedStyle(e).animationName), 'none', 'reduced motion disables entry animation')
          assert.equal(await home.locator('.guide-scroll-hint').evaluate(e => e.tabIndex), -1, 'scroll hint is not an inert button')
          if (width === 390 || width === 1920) await home.screenshot({ path: `/tmp/class-cover-${hasHistory}-${preset}-${width}.png` })
          await home.evaluate(() => window.scrollTo({ top: 150, behavior: 'instant' }))
          await home.waitForFunction(() => getComputedStyle(document.querySelector('.guide-scroll-hint')).opacity === '0')
          await home.locator('.guide-content').scrollIntoViewIfNeeded()
          ...
          if (width === 390 || width === 1920) await home.screenshot({ path: `/tmp/class-content-${hasHistory}-${preset}-${width}.png` })
        }
      }
      for (const viewport of [{ width: 844, height: 390 }, { width: 667, height: 320 }, { width: 390, height: 667 }]) {
        await home.setViewportSize(viewport)
        await home.waitForFunction(height => window.innerHeight === height && window.visualViewport.height === height, viewport.height)
        await home.evaluate(() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))))
        await home.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }))
        const hint = await home.locator('.guide-scroll-hint').boundingBox()
        const brand = await logo.boundingBox()
        assert.ok(hint.y > brand.y + brand.height, 'short viewport separates logo and hint')
        assert.ok(hint.y + hint.height <= viewport.height - 16, `hint fits a short viewport: ${JSON.stringify({ viewport, hint, brand })}`)
        assert.ok((await home.locator('.guide-content').boundingBox()).y >= viewport.height - 1)
      }
      await home.setViewportSize({ width: 1920, height: 900 })
      await home.emulateMedia({ reducedMotion: 'no-preference' })
      assert.equal(await home.locator('.guide-masthead').evaluate(e => getComputedStyle(e).animationIterationCount), '1')
      assert.equal(await home.locator('.guide-scroll-hint p').evaluate(e => getComputedStyle(e).animationDelay), '1s')
