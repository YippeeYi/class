...
const extractsayings = context.window.extractMentionedsayingIds;
...
const cases = [
    ...
    ['[[red:包含 [[saying:sayingId|名言]] 的文字]]', ['inline-red', 'saying-tag']],
...
assert.match(render('{{legacy-saying|旧名言}}'), /saying-tag/);
...
assert.equal([...extractsayings('[[saying:t1|名言]][[del:{{t2|旧名言}}]][[anno:提到 [[saying:t3|名言]]|[[saying:t1|重复]]]]')].sort().join(','), 't1,t2,t3');
assert.equal(extractsayings('普通文本 t1；[[person:t2|不是名言]][[record:t3|不是名言]]').length, 0);
