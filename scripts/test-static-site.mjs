...
assert.match(styles, /\.image-viewer-overlay\s*\{[\s\S]*background: color-mix[\s\S]*backdrop-filter: none;/, 'the image viewer must use a readable dimming surface without a full-viewport blur')
...
assert.doesNotMatch(styles, /--glass-shadow|box-shadow:\s*(?:inset|0\s+(?:0\.[4-9]|1)rem)/, 'materials must not retain independent, inset, or broad direct shadows')
