...
const chronicleLogo = await readFrontend('src/components/archive/chronicle-logo.tsx')
const notFound = await readFrontend('src/pages/not-found-page.tsx')
...
assert.match(index, /<title>编日史<\/title>/, 'the static document title must use the site name')
assert.match(
  index,
...
assert.match(errorBoundary, /返回主页/, 'the root error boundary must return to the real homepage')
...
assert.match(shell, /<SidebarMenuButton[\s\S]*isActive=\{isActive\}/, 'active navigation must use the shadcn SidebarMenuButton state API')
...
assert.match(shell, /const isHomePage = normalizeAppPathname\(location\.pathname\) === '\/'/, 'the root breadcrumb must use its homepage form')
assert.match(shell, /!isHomePage &&[\s\S]*<BreadcrumbSeparator/, 'the homepage breadcrumb must contain only the site name')
assert.doesNotMatch(
  shell.slice(shell.indexOf('const navigation = ['), shell.indexOf('const FULLSCREEN_STORAGE_KEY')),
  /to: '\/'/,
  'the homepage must not remain a sidebar navigation item',
)
...
assert.match(markupContent, /node\.type === 'text'[\s\S]*filterProfanity\(node\.value, hideProfanity\)/, 'profanity filtering must occur on parsed text nodes')
assert.doesNotMatch(markup, /innerHTML|DOMParser/, 'domain markup parser must not use HTML-string rendering')
assert.match(searchIndex, /type: 'material'/, 'search must expose a distinct material result type')
assert.match(searchIndex, /relatedRecords[\s\S]*materialIds/, 'material search must include explicitly related records')
assert.doesNotMatch(searchIndex, /JSON\.stringify/, 'search must never index arbitrary serialized objects')
assert.match(notFound, /<Card[\s\S]*<Empty[\s\S]*404 · 档案未收录[\s\S]*返回主页/, '404 must reuse the responsive shadcn visual system')
...
assert.doesNotMatch(backgrounds, /BoxStyleId|setBoxStyle|box:/, 'removed box-style state must not remain in the appearance controller')
assert.doesNotMatch(themeBootstrap, /dataset\.boxStyle/, 'removed box style must not restore during bootstrap')
...
assert.match(backgroundsPage, /<Label[\s\S]*data-background-id=[\s\S]*appearance-choice group\/background-choice/, 'the visual background boundary must use the shadcn Label as its real hit target')
...
assert.match(recordFilters, /normalizeText\(filterProfanity\(stripMarkup\(record\.content\), hideProfanity\)\)/, 'record-list search must index only filtered record body text')
...
assert.doesNotMatch(imageViewer, /DialogPrimitive|<DialogPortal>|<DialogOverlay>/, 'the image viewer must not bypass the shadcn dialog composition')
assert.match(styles, /\.image-viewer-dialog\[data-slot="dialog-content"\][\s\S]*backdrop-filter: none;[\s\S]*animation: none;/, 'the image viewer CSS must retain the full-viewport paint contract')
...
assert.doesNotMatch(styles, /mask-composite: exclude/, 'removed material layers must not remain at quiz corners')
...
assert.match(quiz, /<ButtonGroup>[\s\S]*<FilterToggle/, 'nearby quiz filters must combine shadcn ButtonGroup with the shared filter control')
assert.match(filterToggle, /<Toggle[\s\S]*variant="outline"/, 'persistent filters must wrap the unmodified shadcn Toggle')
for (const source of [recordFilters, people, search, quiz]) {
  assert.match(source, /<FilterToggle/, 'record, people, search, and quiz filters must share one control')
}
...
assert.doesNotMatch(styles, /box-shadow:\s*0\s+(?:0\.[4-9]|1)rem/, 'surfaces must not retain independent or broad direct shadows')
...
assert.match(imageViewer, /useBackgroundScrollLock\(scrollLockActive\)/, 'all large images must share one background scroll lock through their exit phase')
...
assert.match(shell, /useSelectionMotion<HTMLUListElement>[\s\S]*<SidebarMenu[\s\S]{0,200}ref=\{navigationMotion\.ref\}[\s\S]*<SelectionMotionLayer listItem \/>/, 'sidebar navigation must reuse one measured moving selection layer behind shadcn buttons')
assert.doesNotMatch(shell, /app-sidebar-rail|function AppSidebarRail|<button[^>]+data-sidebar="rail"/, 'the business shell must not duplicate or replace the shadcn rail')
