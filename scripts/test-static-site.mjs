...
assert.match(styles, /\.app-sidebar-navigation > \.app-selection-indicator[\s\S]*background: var\(--sidebar-primary\)/, 'active sidebar items need a clear shared inverse surface')
...
assert.match(backgrounds, /'compact' \| 'default' \| 'glass'/, 'box styles must expose exactly the three product radius families')
assert.doesNotMatch(backgrounds, /id: '(?:soft|round)'/, 'duplicate intermediate radius choices must stay removed')
...
assert.match(styles, /\.record-surface\[data-slot="card"\][\s\S]*--record-rest-border: var\(--glass-content-border\)[\s\S]*border-color: var\(--record-rest-border\)[\s\S]*outline: none/, 'written records need one stable box-model boundary in every palette')
...
assert.match(styles, /\.image-viewer-overlay\s*\{[\s\S]*background: rgb\(8 10 13 \/ 52%\);[\s\S]*backdrop-filter: blur\(14px\)/, 'the image viewer must dim and blur the real current page through one translucent backdrop')
...
assert.match(styles, /\[data-slot="card"\]:not\(\[data-liquid-glass-interactive\]\):not\([\s\S]*?\)[\s\S]*?border: 1px solid var\(--glass-content-border\)[\s\S]*?backdrop-filter: none;/, 'top-level quiz and content cards must reuse one stable glass border without a second backdrop sample')
...
assert.doesNotMatch(backgrounds, /pointermove|elementFromPoint|requestAnimationFrame/, 'liquid material must not run a document-wide pointer sampling loop')
assert.match(quiz, /<ButtonGroup data-liquid-glass-group className="liquid-control-group">/, 'nearby quiz controls must use the shadcn ButtonGroup contract')
assert.match(styles, /\.liquid-control-group:is\(:hover, :focus-within\)[\s\S]*aria-pressed="true"/, 'nearby glass controls must respond through one shared CSS material container')
assert.doesNotMatch(backgrounds, /createAmbientSampler|sampleAmbientColor|--liquid-ambient-local/, 'liquid material must rely on cached theme adaptation instead of per-pointer canvas sampling')
assert.doesNotMatch(styles, /liquid-glass-preview-surface[\s\S]{0,1000}perspective\(/, 'the liquid preview must remain planar rather than simulating a tilted curved slab')
assert.doesNotMatch(styles, /ellipse 48% 115% at var\(--liquid-pointer/, 'functional glass must not use a pointer-centred radial highlight that reads as a convex lens')
...
assert.doesNotMatch(styles, /--glass-shadow|box-shadow:\s*0\s+(?:0\.[4-9]|1)rem/, 'materials must not retain independent or broad direct shadows')
assert.match(styles, /\.app-button:not\(\.app-button--link, :disabled\):active[\s\S]{0,1200}box-shadow:[\s\S]*inset/, 'liquid buttons must communicate press depth through a bounded inner optical response')
assert.doesNotMatch(styles, /:is\(#root \.app-topbar, #root \.app-sidebar \[data-slot="sidebar-inner"\]\)[^{]*\{[^}]*box-shadow:\s*inset/, 'large structural glass surfaces must remain planar and avoid a convex inner rim')
...
assert.doesNotMatch(selectionMotion, /clipPath|borderRadius/, 'selection motion must stay on compositor-friendly transform and opacity properties')
assert.match(selectionMotion, /target\.getBoundingClientRect\(\)[\s\S]*--selection-width[\s\S]*--selection-height/, 'shared selection motion must follow real target geometry instead of assuming equal gap-free steps')
assert.match(backgroundsPage, /app-spatial-selection[\s\S]*data-selection-option/, 'appearance radio grids must reuse the shared moving selection entity')
assert.doesNotMatch(interaction, /pointermove|useState/, 'button press illumination must capture one press origin without per-frame React or pointer work')
