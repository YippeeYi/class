...
assert.match(backgroundsPage, /group\/box flex-col items-stretch gap-0/, 'the shadcn Label used by box choices must preserve a full-width vertical preview')
...
assert.match(backgrounds, /'compact' \| 'default' \| 'rounded'/, 'box styles must expose exactly the three product radius families')
assert.match(backgrounds, /id: 'rounded'[\s\S]*label: '圆角方框'/, 'the former special-material option must be a normal rounded box')
assert.match(themeBootstrap, /dataset\.boxStyle/, 'box style must restore before React starts')
assert.doesNotMatch(`${backgrounds}\n${backgroundsPage}\n${styles}`, /backdropFilter|filter id=|mix-blend-mode/, 'rounded box mode must not introduce a special material runtime path')
...
assert.match(shell, /useSelectionMotion<HTMLUListElement>[\s\S]*<SidebarMenu ref=\{navigationMotion\.ref\}[\s\S]*<SelectionMotionLayer listItem \/>/, 'sidebar navigation must reuse one measured moving selection layer behind shadcn buttons')
