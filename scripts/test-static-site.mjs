...
assert.match(backgroundsPage, /<TabsList className="appearance-section-tabs grid w-full grid-cols-3 gap-1 p-1\.5 group-data-horizontal\/tabs:h-auto">/, 'style section navigation must use one standard shadcn TabsList boundary')
...
assert.match(mealMap, /<Card className="min-h-0 flex-1 gap-0 py-0">[\s\S]*<figure/, 'the map must use the shared shadcn Card boundary')
...
assert.match(backgrounds, /source\.closest<HTMLElement>\(LIQUID_GLASS_GROUPS\)[\s\S]*source\.closest<HTMLElement>\(LIQUID_GLASS_TARGETS\)/, 'grouped shadcn controls must share one liquid material sample before resolving individual children')
...
assert.match(backgrounds, /createAmbientSampler[\s\S]*sampleAmbientColor[\s\S]*--liquid-ambient-local/, 'liquid response must sample the actual local background color instead of using a decorative fixed tint')
