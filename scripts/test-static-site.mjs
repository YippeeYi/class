...
assert.match(backgroundsPage, /<TabsList[\s\S]{0,100}aria-label="风格设置分区"[\s\S]{0,100}className="grid w-full grid-cols-3 group-data-horizontal\/tabs:h-auto"/, 'style section navigation must use the standard shadcn TabsList boundary without a page-specific skin')
assert.doesNotMatch(backgroundsPage, /<TabsList[\s\S]{0,180}(?:bg-card|shadow-sm|backdrop-blur)/, 'style section navigation must not layer a bespoke card surface over TabsList')
...
assert.match(styles, /data-record-jump-highlight="true"[\s\S]*border-color:[\s\S]*box-shadow:/, 'record jump feedback must override the quiet material edge without changing layout')
...
assert.match(styles, /\.image-viewer-overlay[\s\S]*background: color-mix[\s\S]*backdrop-filter: blur/, 'the image viewer must dim and blur the original page instead of painting a black canvas')
...
assert.match(imageViewer, /<Dialog modal="trap-focus"/, 'the full-screen image viewer must trap focus without applying a document scroll lock')
assert.match(styles, /\.image-viewer-overlay[\s\S]*backdrop-filter: none;/, 'the full-screen image overlay must avoid a full-viewport backdrop filter')
