...
assert.match(styles, /\[data-slot="card"\]:not\(\[data-slot="card"\] \[data-slot="card"\]\)[\s\S]*border: 1px solid var\(--glass-content-border\)[\s\S]*backdrop-filter: none;/, 'top-level quiz and content cards must reuse one stable glass border without a second backdrop sample')
