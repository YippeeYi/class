...
try {
  ...
  assert.equal(
    ...
    '隐藏所有记录中的脏话',
    'the homepage preference must expose an accessible label',
  )
  assert.match(await profanityCase.innerText(), /过滤：\*\*\*/)
  assert.doesNotMatch(await profanityCase.innerText(), /傻逼/)
  assert.equal(
    await profanityCase.locator('a.person-link').getAttribute('href'),
    '/person?id=sb',
    'text filtering must not mutate reference identifiers',
  )
  assert.equal(await profanitySwitch.isChecked(), true, 'profanity filtering must default to on')
  await profanitySwitch.click()
  assert.match(await profanityCase.innerText(), /过滤：傻逼/)
  assert.equal(
    await page.evaluate(
      () => JSON.parse(localStorage.getItem('classRecord:contentPreferences:v1') || '{}').hideProfanity,
    ),
    false,
    'the homepage preference must persist immediately',
  )
  await profanitySwitch.click()
  assert.match(await profanityCase.innerText(), /过滤：\*\*\*/)
  ...
  assert.equal(
    await quizPage.locator('#quiz-answer-feedback').innerText(),
    '选择答案或填写完整内容后提交。',
  ...
  const backgroundScrollViewport = page.locator(
    '[data-scroll-area-fixture] [data-slot="scroll-area-viewport"]',
  )
  await backgroundScrollViewport.evaluate((element) => {
    element.scrollTop = 72
  })
  const backgroundScrollBefore = await page.evaluate(() => ({
    windowY: window.scrollY,
    viewportY: document.querySelector(
      '[data-scroll-area-fixture] [data-slot="scroll-area-viewport"]',
    )?.scrollTop,
  }))
  ...
  assert.equal(
    await backgroundScrollViewport.evaluate((element) => element.style.overflow),
    'hidden',
    'opening a large image must lock independent background scroll containers',
  )
  await page.mouse.move(8, 500)
  await page.mouse.wheel(0, 480)
  const backgroundScrollDuring = await page.evaluate(() => ({
    windowY: window.scrollY,
    viewportY: document.querySelector(
      '[data-scroll-area-fixture] [data-slot="scroll-area-viewport"]',
    )?.scrollTop,
  ...
  assert.deepEqual(
    backgroundScrollDuring,
    backgroundScrollBefore,
    'wheel input over a large-image overlay must not move any background scroll position',
  )
  ...
  assert.deepEqual(
    ...
    backgroundScrollBefore,
    'closing a large image must restore every prior background scroll position',
  )
  ...
  const formulaStyles = await page.locator('[data-case="formula-types"]').evaluate(caseElement => {
    ...
    return {
      ...
      mathFontSize: getComputedStyle(text).getPropertyValue('--record-math-font-size').trim(),
  ...
  assert.equal(formulaStyles.mathFontSize, '1em')
  ...
  assert.ok(Math.abs(formulaStyles.fractionSize - formulaStyles.fontSize) < 1, 'fractions must not receive an extra project font scale')
  ...
  assert.ok(nestedFormulaState.blockFormulaSizes.every(size => Math.abs(size - nestedFormulaState.inlineFormulaSizes[0]) < 1), 'block and inline LaTeX share one base font size')
