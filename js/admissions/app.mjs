...
const renderUniversityMarker = async (province, index) => {
  ...
  if (!marker) {
    ...
    const rise = svg('animateTransform', { attributeName: 'transform', type: 'translate', from: `${x} ${y + 18}`, to: `${x} ${y}`, dur: '440ms', fill: 'freeze', calcMode: 'spline', keySplines: '.16 .84 .2 1' }); marker.append(rise); rise.beginElement?.();
  }
...
const moveMarkersToInfo = (province) => {
  const layer = focusLayer(); province.universities.forEach((university, index) => {
    ...
    const transition = svg('animateTransform', { attributeName: 'transform', type: 'translate', from: `${x} ${y}`, to: `${targetX} ${targetY}`, begin: `${index * 150}ms`, dur: '720ms', fill: 'freeze', calcMode: 'spline', keySplines: '.16 .84 .2 1' });
    marker.append(transition); transition.beginElement?.(); marker.dataset.origin = `${x},${y}`;
  });
};
