...
(() => {
    ...
    const ensureSdk = async () => {
        ...
        if (!sdkPromise) {
            sdkPromise = loadScript(SDK_URL).then(() => {
                if (!window.supabase?.createClient) throw new Error("Supabase SDK 不可用。");
    ...
    const getClient = async () => {
        ...
        const config = getConfig();
        if (!isConfigured()) throw new Error("Supabase 尚未配置，请先填写 js/supabaseConfig.js。");
    ...
    const getErrorMessage = (error, fallback) => {
        ...
        if (message.includes("email not confirmed")) return "账号尚未确认，请在 Supabase 中确认该用户。";
        if (message.includes("password")) return "密码不符合要求，请至少使用 6 位字符。";
        if (message.includes("jwt") || message.includes("session")) return "登录状态已失效，请重新登录。";
    ...
    const signIn = async ({ login, password }) => {
        const supabase = await getClient();
        const { data, error } = await supabase.auth.signInWithPassword({ email: normalizeLogin(login), password });
        if (error) throw new Error(getErrorMessage(error, "登录失败。"));
        return data.session;
    };
    ...
    const updatePassword = async (password) => {
        ...
        if (error) throw new Error(getErrorMessage(error, "密码修改失败。"));
    };
    ...
    const getProfile = async () => {
        ...
        const { data, error } = await supabase
            ...
            .eq("id", user.id)
            .maybeSingle();
        if (error) console.warn("读取用户资料失败：", error);
        return {
            id: user.id,
            email: user.email,
