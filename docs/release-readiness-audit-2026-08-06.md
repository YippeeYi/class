...
- 答题：来源→内容→题型三级等权；取消最后可用组合要禁用；人物/名言填空必须以“引用类型 + 实体 ID + 精确可见称呼”定位，不得全局替换同文本；管理员题只在解锁后请求。
...
| 问题 | 根因级结果 | 本轮验证边界 |
| --- | --- | --- |
| 答题挖空下划线 | 题目 AST 只将“类型 + ID + 完全相同可见称呼”节点转成 `blank`；可见字形从首次渲染就保留几何但使用透明前景，揭示只改变涂制，padding、字重、基线、换行均不变 | 自动化覆盖同实体重复称呼、别名、异实体同名、普通同文、名言、句首/中/尾、跨行、嵌套标记、表格与窄屏；真实题库待 A |
