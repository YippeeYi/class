...
1. 确认 `private-exports/` 所在磁盘有足够空间保存线上所有受管 Storage 对象；额外的异地备份仍然建议保留。
...
快照目录受 Git 忽略，格式版本为 2，包含线上数据库 JSON、Storage 路径清单、本次差异以及 `storage/` 下的全部线上二进制对象。只有完整下载成功后脚本才开始上传或写表；任何快照对象缺失都会阻止自动回滚。
...
回滚在任何远端变更前完整校验目标快照，并先为当前线上状态创建一份新的 `pre-rollback` 安全快照。随后恢复全部受管表和 Storage，删除目标快照清单之外的对象。旧格式快照因缺少二进制文件会被明确拒绝，不能自动回滚。
