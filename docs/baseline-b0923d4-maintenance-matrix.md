...
| F06 | 插图有尺寸门 | 非法标记回退原文 | 单个插图错误不阻断正文 | 30×12、深度限制、转义/XSS；正文表格不得横滚或裁切；注释/插图每次打开只取一次指针 X | 分式/箭头以 intrinsic track 定线并保留可见延伸/外距；表格按内容权重换行；浮层中心碰撞后锁定 | typed AST+`MarkupContent` | innerHTML 改 React nodes | 纯 parser+业务 wrapper+Table/HoverCard | 表格、可点击嵌套引用注释、注释/插图首次指针对齐、分式和箭头几何已专项收口 | T/B/P；1280/768/390/320px 极端内容、浮层中心/锁定/边缘、键盘/触控/人物跳转通过，真实私图待 A |
...
| F12 | 题库 Skeleton；切题定位补偿 | 无可生成组合用 Empty | 数据错误与无题不混用 | 取消最后组合被禁用；允许重复抽样 | 未揭示时只渲染固定宽度空位；答案出现前后 min-width、padding、字重和基线不变 | React engine+Button/Card/Alert | 旧版透明答案字形改为不含答案文本的安全占位，同时保留布局稳定性 | 三级随机函数+layout effect+CSS 保形 | 精确等权、无 DOM 答案泄漏、稳定挖空、齐边题头；题体内滚、反馈 footer 常驻 | T/B/R；中英数标点与窄屏契约通过，普通 token 待 A |
...
| F14 | 时间线 Skeleton | 无记录/无有效日期/空月分别 Empty | 数据错误、名言来源错误 Alert | 空月深链保留；刻度超限按步长扩展 | 三层饼图、每日 30–40px 自适应主饼图、图例联动；日格省略重复“日/条/字”单位 | Recharts+Tabs/Card/Badge | 曾缺全局/年度饼图、日饼、精确刻度 | 纯 scale helper+分层 memo+共享来源解析 | 恢复全局/月趋势、年/月饼、日分布与直达；0/个位/两位/百万级日格无溢出 | T/B/P；4/7/10 列日格几何、可读文案与视觉截图通过，真实数据图表交互待 A |
...
| F17 | 当前图 eager、其余 lazy Spinner | 默认背景始终存在 | 单预览独立重试 | 安全协议署名；reduced motion；缓存命中跳过重复解码/采样 | 4:3 真实预览、实际配色主题条、磨砂信息层、560ms 预解码双层淡入 | BackgroundRoot+RadioGroup+AspectRatio | 多页面 bootstrap 改 SPA 根背景 | 外部 bootstrap+palette cache+choice card | 首屏恢复、取色、署名、整卡/方向键选择；零高预览在业务层修正 | T/B/P/R；三图加载、比例、主题条、磨砂、选择与持久化通过 |
