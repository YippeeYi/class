...
| F17 | 当前图 eager、其余 lazy Spinner | 默认背景始终存在 | 单预览独立重试 | 安全协议署名；reduced motion；缓存命中跳过重复解码/采样 | 4:3 真实预览、随景/浅色/深色三组主题预览、主表面分层透明、顶栏/侧栏局部磨砂、560ms 预解码双层淡入 | BackgroundRoot+RadioGroup+AspectRatio | 多页面 bootstrap 改 SPA 根背景；shadcn SidebarInset 默认实色曾遮挡底图 | 同源 appearance bootstrap+palette cache+完整 shadcn token+业务层 shell 表面 | 正式页面持续显示背景；4 浅/3 深预设、首屏恢复、取色、署名、整卡/方向键选择均收口 | T/B/P/R；三图加载、八主题预览/分组/对比度、刷新恢复、背景持续挂载、选择与持久化通过 |
