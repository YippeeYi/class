...
- 风格页顶层“配色 / 背景”使用与其他分段模式一致的共享选中层；各分区内部的配色卡、背景卡
  仍在目标自身更新颜色、边框和选中标记，不跨长距离移动选择框。
- 快速连续选择应立即更新业务状态，不等待动画结束。

## 5. 方框风格

- 正式版本不再保留独立方框风格设置；全部圆角来自 shadcn `--radius` 与 `--radius-sm/md/lg/xl`，业务布局只选择对应 utility。
- 原生滚动条与 shadcn `ScrollArea` 的滑块使用相同的静止、精细指针 Hover 和 Pressed 颜色层级；
  只改变绘制颜色，不改变滑块尺寸或滚动位置。
...
- 背景选择器只渲染 640px 级独立缩略图。已选背景先以缩略图完成首屏绘制，再异步解码原始 WebP；
  切换时保留同一缩略图作为高清图淡入的底层，解码失败则稳定回退到缩略图。
- 手写页、地图、答题插图、记录插图及存储路径形式的人物头像先请求 Supabase 转换后的限宽预览；
  只有打开共享 `ImageViewer` 时才签名和解码原图，原图就绪前不得移除已经可用的预览。
- 预览签名 Promise、已解析签名地址、图片尺寸和已解码原图都按资源与变体去重；仍有效的地址在 SPA
  页面往返时同步复用，访问权限清理或页面退出时必须清空私有运行时缓存。
- 当前视口内明确需要的预览使用高优先级，相邻页预取和启动后的背景升级使用低优先级；用户主动打开
  大图或选择新背景后，原图加载才提升为高优先级。
- 所有媒体框在请求前提供已知宽高、数据库尺寸、持久尺寸缓存或稳定后备比例，禁止因图片加载导致明显
  布局跳动；静态缩略图和背景资源由部署缓存头复用。
...
- 记录 annotation 字段使用非模态 shadcn Popover，沿用注解标记的方框样式，完整调用 MarkupContent。父容器滚动时关闭；浮层自身可滚动，嵌套图片继续由统一查看器处理。
