...
61. 让普通填空答案字形在隐藏状态继续占据真实宽度，并取消揭示时的字重变化。
62. 消除 shadcn Card 默认纵向 padding 造成的题型色面顶部缝隙，同时保持 Card 基础组件只读。
63. 恢复隐藏题“只暴露入口、不自动选中”的筛选语义。
64. 移除上一题排除分支，恢复来源、内容、题型三级严格等权，并以注入随机源执行确定性测试。
65. 恢复时间线整体记录人饼图、跨年份月趋势、年度记录人饼图和月度记录人饼图。
66. 恢复旧版月/日固定刻度与超限步长扩展规则，抽取为纯领域函数。
67. 恢复每日记录人小饼图、稳定颜色、重要量和键盘/指针图例联动。
68. 合法空月 URL 保持空结构，不再被自动替换为有记录月份。
69. 将名言来源解析抽为共享模块，名言、搜索、时间线遵循同一 direct/fallback/唯一性规则。
70. 建立 22 个功能域、每项 20 字段的可维护基线矩阵，并明确登录态验证边界。

状态：61–70 已实现。类型、Biome、现有 11 组命令链和新增的 Vite SSR 纯函数运行断言通过；生产构建与浏览器几何复核在最终门禁执行。
