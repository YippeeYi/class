...
| 人物/名言挖空 | 旧版用纯文本 `replace` 仅命中首个同名字符串；迁移版又曾先按 ID 把所有别名压成答案，再对所有文本 `split`，会误伤普通同文和异实体同名 | 保留原始结构，仅将同引用类型、同实体 ID、同精确可见称呼的 AST 节点转成安全 `blank`；重复节点同步挖空，别名/异实体/普通同文保留，提交后原位显示 | A/D |
...
## 13. 第九次全面整改：工作区、背景与私有地图

本轮重新以实际 DOM、状态所有权和多视口浏览器结果审查此前的“已完成”结论，并修正了文档与代码不一致处：

| 专项 | 最终实现 | 验证结论 |
| --- | --- | --- |
| 致谢入口 | 删除全局右下 fixed 入口，Sidebar/移动菜单保留唯一同源入口 | 全部重点路由无固定致谢元素 |
| 工作区滚动 | `/materials`、`/quiz`、`/map` 统一锁定根视口；滚动职责下放到明确的 `ScrollArea` | 五档视口均无 outer X/Y overflow；资料 2 个、答题 1 个、地图 0 个内部滚动区 |
| 背景选择与根层 | 使用 shadcn RadioGroup choice card；整卡、方向键可选；新背景预解码后与旧层交叉淡出 | 点击“山”后方向键切到“云”，单一 checked，零控制台警告 |
| 资料排版 | 双 Card 改为单一目录/正文工作区，正文增大到 17–18px/1.92 行高并扩大阅读宽度 | 1920 至 390 宽度无按钮越界，目录与正文独立滚动 |
| 蹭饭图状态 | 元数据只提供 4838×2721 尺寸，签名 Hook 唯一拥有 URL；取消第二次签名 Range 探测；Hook 按 path 隔离异步状态 | 真实私有 PNG 五档视口 `complete=true`，自然尺寸一致；对象、路径、MIME、签名读取均正确 |
| 蹭饭图与答题布局 | 地图使用单一剩余高度 figure；答题使用紧凑筛选、内部题体滚动、常驻反馈 footer | 地图 contain 无裁切；答题换题不带走外层滚动，关键操作保持可见 |
| Shadcn 边界 | 所有改动位于业务页面、布局、Hook、服务与业务样式 | `frontend/src/components/ui` 零差异 |

最终自动验证为 TypeScript、Biome、11 组回归测试和 Vite 8 生产构建全部通过。普通最终用户邀请码在本轮不可用，因此真实数据浏览器回归在测试层绕过了邀请码刷新/RLS；它不替代普通 token 的完整授权链路。统一的最终状态、后端核验、测试矩阵和唯一剩余手工复核步骤见 `legacy-baseline-audit.md`。
