...
| `/` | 主页。显示记录、人物、名言数量，提供核心入口和工具入口、脏话显示偏好；当天存在历史记录时显示“历史上的今天”。 |
| `/records` | 记录浏览。普通状态固定为列表模式；管理员在本页输入授权序列后可浏览隐藏记录并切换其列表/书面模式。 |
...
| `/search?q=...` | 搜索。120ms 防抖，按相关度搜索记录、箴言、页补充、人物、名言和资料；资料正文及引用资料的关联记录可命中，结果按内容类型区分。 |
...
| `/backgrounds` | 风格设置。独立组合配色和背景，选择即时生效并持久化。 |
...
- 根级错误边界会接管渲染异常和 lazy chunk 下载失败，提供重新加载/返回主页恢复入口；仅在当前标签页保存诊断编号、故障类型、构建模式、路径和时间，不记录查询参数、fragment、正文或访问凭证。
...
| 参数 | 含义 |
| --- | --- |
| `view=written` | 仅在管理员已于本页输入 `qibaishihuaxia`、进入隐藏记录界面后生效；其他情况会被规范化为列表模式。 |
...
- 搜索只索引面向用户的记录正文/日期/记录人/附件名、页箴言、页补充、人物资料、名言，以及资料标题、正文和引用资料的关联记录正文；不把内部 ID、资源路径或任意 JSON 字段混入索引。结果先按标题完全匹配、前缀、包含、正文包含分级，再按时间/稳定键排序，不做静默截断。
...
- 记录 JSON 加载后会扫描正文中的媒体标记，并读取同一发布流程生成的受保护尺寸索引；索引按公开与隐藏内容分开，尺寸在页面间共享并去重。缺失尺寸才通过有并发上限的预览图 Range 请求或视频 metadata 补读，接近视口时才加载缩略图，打开大图时再加载原图。
- 正文图片按原比例显示为当前文字两行行高，极宽图片受容器宽度约束；已知尺寸时图片和视频在预览加载前保持最终比例，加载期间仅显示居中的圆环。含图片的表格优先为媒体列保留可读宽度，窄屏时只允许表格区域横向滚动。
- 大图查看等既有图片组件解码失败最多自动强制重签一次；正文缩略图加载失败时仅在该媒体节点显示错误状态。
- 注解使用 shadcn HoverCard；正文插图直接显示缩略图，点击后使用统一大图查看器。
- 大图查看器组合 shadcn Dialog、Tooltip 和 Button，支持 1×–8× 缩放、滚轮/双击/双指缩放、拖动、方向键移动、复位和焦点锁定；打开期间统一冻结 document 与页面内独立滚动容器，关闭时恢复原位置和样式。
...
内容偏好单独保存在 `classRecord:contentPreferences:v1`。主页“隐藏脏话”默认开启，只在解析完成的人类可读文本节点上动态替换为 `***`，不修改源 JSON，也不处理标记 ID、参数和资源路径。
...
私有二进制对象位于非公开 `classrecord-private` bucket。普通会话可访问 `data/attachments/` 和 `images/private/meal-map.png`；全部 `images/record-pages/`、`hidden/` 与 `images/quiz/` 路径均要求管理员权限。源 JSON 不上传到 Storage。
...
3. 页面专属数据在进入页面后再加载：书面页、资料、隐藏题、地图元数据、风格资源和致谢不会由主页全量预取。
...
| 注解 / 插图 / 视频 / 公式 | `[[anno:注解内容|被注释文字]]` / `[[illu:example.png]]` / `[[video:example.mp4]]` / `[[latex:x^2+y^2]]` |
| 样式 | `[[del:删除线]]`、`[[under:下划线]]`、`[[red:标红]]`、`[[hide:黑幕]]` |
| 对齐 | `[[center:居中]]`、`[[right:右对齐]]` |
